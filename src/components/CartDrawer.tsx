    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full bg-card border-border">
        <SheetHeader className="flex-shrink-0">
          <SheetTitle className="font-display text-2xl tracking-wider">Carrinho</SheetTitle>
          <SheetDescription>
            ...
            <>
                ...
                {items.map((item) => (
                  <div key={item.variantId} className="flex gap-4 p-3 rounded-lg bg-secondary/30">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="font-semibold text-primary text-sm mt-1">{formatPrice(item.price.amount, item.price.currencyCode)}</p>
                    </div>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t border-border">
                <div className="flex justify-between items-center">
                  <span className="text-lg font-display tracking-wider">Total</span>
                  <span className="text-xl font-bold text-primary">{formatPrice(totalPrice.toString(), currency)}</span>
                </div>
