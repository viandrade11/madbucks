...
const SaboneteLiquidoLP = ({ product }: SaboneteLiquidoLPProps) => {
  ...
  return (
    <>
      ...
      <section className="section-padding bg-muted/30"><div className="container mx-auto px-4"><ScrollReveal><div className="text-center mb-12"><p className="text-xs font-bold uppercase tracking-[0.3em] text-muted-foreground mb-3">Rotina Completa</p><h2 className="font-display text-2xl md:text-3xl tracking-tight text-foreground">Complete Sua Rotina</h2></div></ScrollReveal><div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">{[{ handle: "madbucks-creme-hidratante-tattoo-69667a5124762", title: "Creme Hidratante Tattoo", step: "Passo 2 — Hidrate", desc: "Hidratação profunda de 24h" }, { handle: "madbucks-tattoo-balm-stick-69668baa49da0", title: "Balm Stick", step: "Passo 3 — Proteja", desc: "Proteção portátil" }, { handle: "madbucks-tattoo-intensify", title: "Tattoo Intensify", step: "Passo 4 — Intensifique", desc: "Intensifica cores e contraste" }].map((item, i) => (<ScrollReveal key={item.handle} delay={i * 0.1}><Link to={`/produto/${item.handle}`} className="border border-border rounded p-6 text-center space-y-3 hover:border-foreground transition-colors group block"><p className="text-[10px] font-bold uppercase tracking-[0.3em] text-muted-foreground">{item.step}</p><h3 className="text-sm font-extrabold text-foreground group-hover:underline">{item.title}</h3><p className="text-xs text-muted-foreground">{item.desc}</p><span className="text-xs font-bold uppercase tracking-wider text-foreground inline-flex items-center gap-1">Ver produto <ArrowLeft className="h-3 w-3 rotate-180" /></span></Link></ScrollReveal>))}</div></div></section>
      ...
      <section className="py-20 bg-foreground text-background"><div className="container mx-auto px-4 text-center space-y-6"><ScrollReveal><h2 className="font-display text-2xl md:text-3xl tracking-tight">O Primeiro Passo da Rotina.</h2><p className="text-sm opacity-60 max-w-md mx-auto">pH balanceado. Sem sulfatos. Respeita sua tinta.</p><Button variant="secondary" className="rounded-none h-14 px-12 text-xs uppercase tracking-[0.2em] font-bold bg-background text-foreground hover:bg-background/90 gap-2" onClick={handleAddToCart} disabled={isLoading}>{isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <ShoppingCart className="h-4 w-4" />}Comprar Agora — {selectedVariant ? formatPrice(selectedVariant.price.amount, selectedVariant.price.currencyCode) : ""}</Button></ScrollReveal></div></section>
    </>
  );
};
