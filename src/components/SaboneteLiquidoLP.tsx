...
import {
  ...
  Droplets, Shield, Sparkles, Leaf, Waves
} from "lucide-react";
...
const BENEFITS_DETAILED = [
  {
    icon: Waves,
    title: "pH Balanceado 5.5",
    desc: "A mesma acidez natural da pele saudável. Limpa sem alterar o equilíbrio cutâneo e sem irritar áreas tatuadas sensíveis.",
  },
  {
    icon: Shield,
    title: "Livre de Sulfatos Agressivos",
    desc: "Sem SLS, SLES ou outros sulfatos que ressequem a pele e degradam os pigmentos da tatuagem. Limpeza eficiente com espuma suave.",
  },
  {
    icon: Leaf,
    title: "Extratos Naturais Calmantes",
    desc: "Formulado com extratos botânicos que acalmam a pele e preparam a superfície para absorver melhor os produtos seguintes da rotina.",
  },
  {
    icon: Sparkles,
    title: "Prepara Para a Hidratação",
    desc: "A limpeza correta é o primeiro passo. Ao remover impurezas sem agredir, o sabonete prepara a pele para absorver o creme hidratante de forma mais eficaz.",
  },
];
...
const INGREDIENTS = [
  {
    ...
    benefits: ["Propriedades anti-inflamatórias", "Acalma a pele irritada", "Suaviza e refresca"],
  },
  {
    name: "Glicerina Vegetal",
    benefits: ["Umectante natural", "Atrai e retém umidade na pele", "Previne o ressecamento pós-limpeza"],
  },
  {
    name: "Extrato de Aloe Vera",
    benefits: ["Regenerador natural", "Hidrata durante a limpeza", "Protege a barreira cutânea"],
  },
  {
    name: "Tensoativos Suaves",
    benefits: ["Limpeza eficiente sem agressão", "Espuma cremosa e confortável"],
  },
];

const HOW_TO_STEPS = [
  { num: "01", title: "Molhe a pele", desc: "Molhe a área tatuada com água morna. Evite água muito quente, que pode ressecar a pele e afetar os pigmentos." },
  { num: "02", title: "Faça espuma", desc: "Aplique uma pequena quantidade do sabonete e faça espuma com as mãos. A fórmula suave cria uma espuma cremosa." },
  { num: "03", title: "Massageie suavemente", desc: "Massageie sobre a tatuagem com movimentos suaves. Não esfregue — a limpeza eficiente não precisa de atrito." },
  { num: "04", title: "Enxague e seque", desc: "Enxague bem com água morna e seque a pele com toques leves usando uma toalha macia. Nunca esfregue." },
];

const FAQS = [
  { q: "Posso usar todos os dias?", a: "Sim, o sabonete foi formulado para uso diário. A fórmula suave não resseca nem irrita a pele, mesmo com uso frequente." },
  { q: "Faz espuma?", a: "Sim, faz uma espuma suave e cremosa. A fórmula sem sulfatos agressivos produz menos espuma que sabonetes comuns, mas limpa igualmente bem." },
  { q: "Tem fragrância?", a: "Fragrância leve e natural, derivada de extratos botânicos. Sem componentes sintéticos que possam irritar a pele ou degradar a tinta." },
  { q: "Posso usar no corpo todo?", a: "Sim, o sabonete é suave o suficiente para uso corporal completo. Porém, foi especificamente otimizado para áreas tatuadas." },
  { q: "É indicado para tatuagens recém-feitas?", a: "Sim, o sabonete é suave o suficiente para usar durante a cicatrização. Consulte seu tatuador sobre o momento ideal para iniciar a limpeza." },
  { q: "Substitui meu sabonete normal?", a: "Pode substituir nas áreas tatuadas. Para o restante do corpo, você pode continuar com seu sabonete habitual ou usar o nosso — é seguro para toda a pele." },
  { q: "Quanto tempo dura cada frasco?", a: "Com uso diário, cada frasco dura em média 45-60 dias." },
  { q: "Por que não posso usar sabonete comum?", a: "Muitos sabonetes contêm sulfatos agressivos, álcool e fragrâncias sintéticas que ressequem a pele e aceleram o desbotamento dos pigmentos. O Sabonete Madbucks foi formulado para limpar sem esses riscos." },
];

interface SaboneteLiquidoLPProps {
    ...
    <>
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="text-sm text-muted-foreground leading-relaxed">
                  O primeiro passo da rotina Madbucks. Limpeza suave com pH balanceado que respeita os pigmentos e prepara a pele para os próximos cuidados.
                </p>
              </div>
              <div className="flex flex-wrap gap-x-6 gap-y-2">
                {[
                  { icon: Waves, text: "pH Balanceado" },
                  { icon: Leaf, text: "Sem sulfatos" },
                  { icon: Droplets, text: "Espuma suave" },
                ].map((badge, i) => (
      ...
      <section className="section-padding">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <p className="text-xs font-bold uppercase tracking-[0.3em] text-muted-foreground mb-3">O Primeiro Passo</p>
            <h2 className="font-display text-2xl md:text-3xl tracking-tight text-foreground">Limpeza Que Respeita Sua Tinta</h2>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            <div className="space-y-8">
              <div>
                ...
                <h2 className="font-display text-2xl md:text-3xl tracking-tight text-foreground">Limpeza Inteligente Para Tattoo</h2>
              </div>
            ...
            <div className="aspect-[4/5] overflow-hidden rounded bg-muted flex items-center justify-center">
              {images[1]?.node ? (
                <img src={images[1].node.url} alt="Sabonete Líquido Madbucks" className="w-full h-full object-cover" loading="lazy" />
              ) : images[0]?.node ? (
                <img src={images[0].node.url} alt="Sabonete Líquido Madbucks" className="w-full h-full object-cover" loading="lazy" />
              ) : (
      ...
      <section className="bg-foreground text-background py-5">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            {["pH 5.5", "SEM SULFATOS", "ESPUMA SUAVE", "FRAGRÂNCIA NATURAL"].map((item, i) => (
      ...
      <section className="section-padding">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            ...
            <p className="text-sm text-muted-foreground max-w-xl mx-auto">Fórmula suave com extratos naturais que limpam sem agredir a pele tatuada.</p>
          </div>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            <div className="order-2 lg:order-1 aspect-[4/5] overflow-hidden rounded bg-muted flex items-center justify-center">
              {images[2]?.node ? (
                <img src={images[2].node.url} alt="Como usar o Sabonete" className="w-full h-full object-cover" loading="lazy" />
              ) : images[0]?.node ? (
                <img src={images[0].node.url} alt="Como usar o Sabonete" className="w-full h-full object-cover" loading="lazy" />
              ) : (
                <span className="text-muted-foreground text-sm">Sem imagem</span>
              )}
            </div>
            <div className="order-1 lg:order-2 space-y-8">
              <div>
                ...
                <h2 className="font-display text-2xl md:text-3xl tracking-tight text-foreground">Molhe, Ensaboe, Enxague.</h2>
                <p className="text-sm text-muted-foreground mt-3">A limpeza correta é a base de toda rotina. Sem esfregar, sem agredir.</p>
              </div>
              <div className="space-y-6">
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {[
              ...
              { handle: "madbucks-tattoo-intensify", title: "Tattoo Intensify", step: "Passo 4 — Intensifique", desc: "Intensifica cores e contraste" },
            ].map((item) => (
      ...
      <section className="py-20 bg-foreground text-background">
        <div className="container mx-auto px-4 text-center space-y-6">
          <h2 className="font-display text-2xl md:text-3xl tracking-tight">A Limpeza Que Sua Tattoo Merece.</h2>
          <p className="text-sm opacity-60 max-w-md mx-auto">pH balanceado. Sem sulfatos. O primeiro passo para tatuagens vivas.</p>
